...
import { LifeBuoy, Code, Palette, Users, BookOpen, GitCommit, Monitor, ChevronDown, ChevronRight, Zap, FlaskConical, BrainCircuit, Home } from 'lucide-react';
...
const toolSections = [
  {
    ...
    tools: [
      { 
        title: 'Dashboard', 
        icon: Home,
        description: 'Your central hub for project selection, high-level statistics, and quick navigation. All roads lead from here.',
        features: [
      ...
      { 
        title: 'Lab Assistant', 
        icon: FlaskConical,
        description: 'An interactive AI chat interface for general ROM hacking questions and assistance.',
        features: [
  ...
  },
  {
    ...
    tools: [
      { 
        title: 'Flag Forge', 
        icon: Zap,
        description: 'A comprehensive database for managing game flags, blending vanilla flags with your custom creations.',
        features: [
      ...
      { 
        title: 'Script Sage', 
        icon: Code,
        description: 'An advanced integrated development environment (IDE) for HMA/XSE scripting with powerful AI assistance.',
        features: [
      ...
      { 
        ...
        icon: Users,
        description: 'AI-powered tool for generating strategically sound and thematically consistent NPC trainers.',
        features: [
  ...
  },
  {
    ...
    tools: [
      { 
        title: 'Sprite Studio', 
        icon: Palette,
        description: 'A utility for validating and managing your custom sprites to ensure compatibility with Gen III games.',
        features: [
      ...
      { 
        ...
        icon: BookOpen,
        description: 'A suite of tools for story writers to track events, manage characters, and generate dialogue.',
        features: [
  ...
  },
  {
    ...
    tools: [
      { 
        title: 'Bug Catcher', 
        icon: GitCommit,
        description: 'A detailed, filterable changelog that automatically records every significant action taken across the app.',
        features: [
      ...
      { 
        title: 'Preview Tab', 
        icon: Monitor,
        description: 'An integrated emulator and debugger for live testing and analysis of your ROM hack (BETA).',
        features: [
  ...
  },
  {
    ...
    tools: [
       { 
        title: 'Knowledge Hub', 
        icon: BrainCircuit,
        description: 'Your project\'s personal, searchable encyclopedia. A place to store notes, code snippets, and discoveries.',
        features: [
...
function ToolExpander({ tool, color }) {
  ...
  return (
    <Card className="bg-white/90 dark:bg-slate-900/80 border border-slate-300 dark:border-slate-700/50 shadow-sm transition-all duration-300 hover:border-slate-400 dark:hover:border-slate-600">
      <CardHeader 
        className="p-4 flex flex-row items-center justify-between cursor-pointer" 
        onClick={() => setIsOpen(!isOpen)}
      >
        <div className="flex items-center gap-4">
          <div className={`p-3 bg-gradient-to-br from-${color}-500/20 to-slate-500/10 rounded-lg`}>
            <tool.icon className={`w-6 h-6 text-${color}-400`} />
          </div>
          <div>
            <CardTitle className="text-lg text-slate-900 dark:text-white">{tool.title}</CardTitle>
            ...
            <CardContent className="pt-0 p-4 pl-6 border-t border-slate-200 dark:border-slate-700/50">
              <div className="ml-12 mt-4">
                ...
                <Link to={createPageUrl(tool.title.replace(' ', ''))}>
                  <Button variant="outline" className={`mt-4 border-${color}-400/50 text-${color}-400 hover:bg-${color}-400/10 hover:text-${color}-300`}>
                    Go to {tool.title}
                  </Button>
                </Link>
              </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-8">
          {toolSections.map((section, sectionIndex) => (
            ...
            >
              <div className="mb-4">
                <h2 className={`text-2xl font-semibold text-${section.color}-400 mb-2`}>
                  {section.title}
                </h2>
                <div className={`w-16 h-1 bg-gradient-to-r from-${section.color}-400 to-${section.color}-600 rounded-full`}></div>
              </div>
        ...
        <motion.div 
          ...
          className="mt-12"
        >
          <Card className="bg-white/90 dark:bg-slate-900/80 backdrop-blur-sm border border-slate-300 dark:border-slate-800">
            <CardHeader>
            ...
            <CardContent className="space-y-4 text-slate-600 dark:text-slate-400">
              <p>
                Welcome to EmeraldMind! This comprehensive suite is designed to accelerate your Generation III ROM hacking workflow with AI-powered assistance and professional-grade tools.
              </p>
              <div className="bg-cyan-500/10 border border-cyan-400/30 rounded-lg p-4">
                <h4 className="font-semibold text-cyan-400 mb-2">Quick Start Guide:</h4>
                <ol className="space-y-2 text-sm">
                  ...
                  <li className="flex items-start gap-2">
                    ...
                    Test everything in the Preview Tab and track issues with the Bug Catcher
                  </li>
                </ol>
              </div>
            </CardContent>
