...
export default function BugCatcher() {
  ...
  const loadChangelog = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const changelogEntries = await ChangelogEntry.list('-created_date');
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <Card className="bg-white/90 dark:bg-slate-900/80 backdrop-blur-sm border border-slate-300 dark:border-slate-800 rounded-2xl shadow-xl">
          <CardHeader className="flex flex-row items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800">
            <CardTitle className="text-xl text-slate-900 dark:text-white">Project History</CardTitle>
          ...
          <CardContent className="p-4">
            {isLoading ? (
              ...
              <EntityErrorHandler error={loadError} entityName="Changelog" onRetry={retryLoad} />
            ) : (
              <div className="space-y-4">
                {entries.map(entry => (
                  <div key={entry.id} className="p-3 rounded-lg bg-slate-100 dark:bg-slate-800/50">
                    <div className="flex justify-between items-center mb-1">
                      <div className="flex items-center gap-3">
                        <Badge className={`${actionColors[entry.action] || 'bg-slate-500/20 text-slate-400'} uppercase`}>
                          {entry.action}
                        </Badge>
                        <span className="font-semibold text-slate-900 dark:text-white">{entry.item_name}</span>
                        <span className="text-sm text-slate-500 dark:text-slate-400">in {entry.module}</span>
                      </div>
