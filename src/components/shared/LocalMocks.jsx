...
export const InvokeLLM = async ({ prompt, add_context_from_internet = false, response_json_schema = null }) => {
  console.warn('🔄 InvokeLLM called locally - this will be handled by Ollama integration');
...
export const GenerateImage = async ({ prompt }) => {
  ...
  return { 
    url: `https://via.placeholder.com/512x512/6366f1/ffffff?text=${encodeURIComponent(prompt.substring(0, 20))}` 
  };
};
...
const createMockEntity = (entityName) => {
  ...
  return {
    list: async (sort = '-created_date', limit = 100) => {
      console.warn(`📊 ${entityName}.list called locally - returning mock data`);
      return Array.from(mockData.values())
        .sort((a, b) => sort.startsWith('-') ? b.created_date - a.created_date : a.created_date - b.created_date)
        .slice(0, limit);
    },
    
    filter: async (criteria, sort = '-created_date', limit = 100) => {
      console.warn(`🔍 ${entityName}.filter called locally - returning mock data`);
    ...
    update: async (id, data) => {
      console.warn(`📝 ${entityName}.update called locally`);
